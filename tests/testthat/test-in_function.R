...
test_that("When in a function parenthesis starting at the first argument the function data is returned",{
  env$linebuffer <- 'utils::citation()'
  env[["start"]] <- 16L

  inFun <- in_function(env)

  expect_equal(.DollarNames(inFun),c("currentFunction","currArgs","currentArg","IsFirstArg"))
...
test_that("When in a function parenthesis at the second argument the function data is returned",{
  env$linebuffer <- 'utils::citation("base",)'
  env[["start"]] <- 23L
  ...
  expect_null(inFun$currArgs$lib.loc)
  expect_null(inFun$currArgs$auto)
  expect_false(inFun$IsFirstArg)
  expect_equal(inFun$currentArg, "lib.loc")
})
